...
import {
  Activity, Search, Trash2, Download, Filter, Info, AlertTriangle,
  XCircle, CheckCircle2, Pause, Play, ChevronDown
} from 'lucide-react';
...
export default function Logs() {
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = `edward-md-logs-${Date.now()}.txt`;
    a.click();
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-text-muted mt-1">
            {visibleLogs.length} entries • {paused ? 'Paused' : 'Live'}
          </p>
        </div>
        <div className="flex items-center gap-2">
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-2 text-xs font-medium rounded-lg transition-colors ${
              paused
                ? 'bg-wa-green/10 border border-wa-green/20 text-wa-green'
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 bg-accent-blue/10 border border-accent-blue/20 text-accent-blue text-xs font-medium rounded-lg hover:bg-accent-blue/20 transition-colors"
          >
          ...
          {(['all', 'info', 'success', 'warn', 'error'] as const).map(level => (
            <button
              ...
              className={`px-3 py-2 text-xs font-medium rounded-lg transition-colors flex items-center gap-1 ${
                filter === level
                  ? level === 'all' ? 'bg-wa-green/15 text-wa-green border border-wa-green/30'
                    : `${levelConfig[level as keyof typeof levelConfig].bg} ${levelConfig[level as keyof typeof levelConfig].color} border ${levelConfig[level as keyof typeof levelConfig].border}`
      ...
      >
        <div className="bg-bg-input/50 px-4 py-2 border-b border-border flex items-center justify-between">
          ...
          <button
            ...
            className={`text-xs flex items-center gap-1 ${autoScroll ? 'text-wa-green' : 'text-text-muted'}`}
          >
            <ChevronDown className="w-3 h-3" />
            Auto-scroll {autoScroll ? 'ON' : 'OFF'}
          </button>
        </div>
        <div className="max-h-[500px] overflow-y-auto p-2 space-y-1 font-mono">
          {visibleLogs.length === 0 ? (
            <div className="text-center py-16">
              <Activity className="w-10 h-10 text-text-muted mx-auto mb-3 opacity-50" />
              <p className="text-text-muted text-sm">No log entries to display</p>
              <p className="text-text-muted text-xs mt-1">Logs will appear here as the bot processes messages</p>
            </div>
          ) : (
            visibleLogs.map(log => {
              ...
              return (
                <div
                  key={log.id}
                  className={`flex items-start gap-2 px-3 py-2 rounded-lg ${config.bg} border ${config.border} animate-fade-in`}
                >
                  ...
                  <div className="min-w-0 flex-1 flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3">
                    ...
                    {log.source && (
                      <span className={`text-[10px] ${config.color} shrink-0 px-1.5 py-0.5 rounded ${config.bg}`}>
                        {log.source}
                      </span>
                    )}
