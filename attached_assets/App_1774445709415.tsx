        ...
        <div className="max-h-80 overflow-y-auto">
          {notifications.length === 0 ? (
            ...
            notifications.slice(0, 20).map(n => (
              <div
                key={n.id}
                className={`px-4 py-3 border-b border-border/50 ${!n.read ? 'bg-bg-card/50' : ''} hover:bg-bg-card/30 transition-colors`}
              >
                <div className="flex items-start gap-2.5">
                  ...
                  {!n.read && (
                    <button
                      ...
                      className="text-text-muted hover:text-wa-green transition-colors shrink-0"
                    >
...
function Sidebar() {
  ...
  return (
    <>
      ...
      <aside className={`fixed top-0 left-0 h-full z-50 flex flex-col bg-bg-secondary border-r border-border transition-all duration-300 ${
        ...
        {sidebarOpen && currentUser && (
          <div className="px-3 py-3 border-b border-border animate-fade-in">
            <button
              onClick={() => { setPage('profile'); if (window.innerWidth < 1024) setSidebarOpen(false); }}
              className="w-full flex items-center gap-2.5 p-2 rounded-xl hover:bg-bg-card transition-all"
            >
          ...
          {navItems.map(item => {
            ...
            return (
              ...
              >
                ...
                {sidebarOpen && item.id === 'logs' && (
                  <span className="ml-auto text-[10px] bg-wa-green/20 text-wa-green px-1.5 py-0.5 rounded-full">
                    Live
                  </span>
                )}
              </button>
          ...
          <div className="px-3 py-3 border-t border-border animate-fade-in">
            <div className="glass rounded-xl p-3 space-y-2">
              <div className="flex justify-between text-[10px]">
                ...
                <span className="text-wa-green font-mono">{formatUptime(stats.uptime)}</span>
              </div>
              <div className="flex justify-between text-[10px]">
                <span className="text-text-muted">Response</span>
                <span className="text-accent-blue font-mono">{stats.avgResponseTime}ms</span>
              </div>
              <div className="flex justify-between text-[10px]">
                <span className="text-text-muted">CPU</span>
                <span className={`font-mono ${stats.cpuUsage > 80 ? 'text-accent-red' : 'text-wa-green'}`}>{stats.cpuUsage}%</span>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="p-3 border-t border-border shrink-0">
          <button
            onClick={logout}
            className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-accent-red hover:bg-accent-red/10 transition-all"
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div className="hidden sm:flex items-center gap-2">
            {isConnected ? (
              <div className="flex items-center gap-1.5 bg-wa-green/10 border border-wa-green/20 text-wa-green px-2.5 py-1 rounded-lg text-xs font-medium">
                <Wifi className="w-3 h-3" />
                Connected
              </div>
            ) : (
              <div className="flex items-center gap-1.5 bg-accent-red/10 border border-accent-red/20 text-accent-red px-2.5 py-1 rounded-lg text-xs font-medium">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="hidden md:flex items-center gap-4 mr-4">
            <div className="text-right">
              ...
              <p className="text-xs font-semibold text-text-primary tabular-nums">{stats.messagesReceived.toLocaleString()}</p>
            </div>
            <div className="w-px h-6 bg-border" />
            <div className="text-right">
              <p className="text-[10px] text-text-muted">Commands</p>
              <p className="text-xs font-semibold text-text-primary tabular-nums">{stats.commandsExecuted.toLocaleString()}</p>
            </div>
          </div>
          ...
          <div className="relative">
            <button
              ...
              className="relative p-2 rounded-lg text-text-secondary hover:text-text-primary hover:bg-bg-card transition-all"
            >
              <Bell className="w-5 h-5" />
              {unreadCount > 0 && (
                <div className="absolute top-1 right-1 min-w-[16px] h-4 bg-accent-red rounded-full flex items-center justify-center">
                  <span className="text-[9px] text-white font-bold px-1">{unreadCount > 9 ? '9+' : unreadCount}</span>
                </div>
              )}
            </button>
          ...
          {currentUser && (
            <div className="flex items-center gap-2 bg-bg-card border border-border rounded-xl px-3 py-1.5">
              ...
              <div className="hidden sm:block">
                <p className="text-xs font-medium text-text-primary">{currentUser.displayName}</p>
      ...
      <div className={`transition-all duration-300 ${sidebarOpen ? 'lg:ml-64' : 'lg:ml-20'}`}>
        ...
        <footer className="px-6 py-4 border-t border-border text-center">
          <p className="text-xs text-text-muted flex items-center justify-center gap-1.5">
            ...
            <a href="https://github.com/WhiskeySockets/Baileys" target="_blank" rel="noopener noreferrer" className="text-wa-green hover:underline inline-flex items-center gap-0.5">
              Baileys <ExternalLink className="w-2.5 h-2.5" />
            </a>
          </p>
        </footer>
      </div>
