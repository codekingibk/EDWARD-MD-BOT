      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-text-muted mt-1">
            {enabledInCategory}/{totalInCategory} enabled • {plugins.length} total plugins
          </p>
        </div>
          ...
          return (
            ...
            >
              ...
              <span className={`px-1.5 py-0.5 rounded text-[10px] ${
                isActive ? 'bg-wa-green/20' : 'bg-bg-input'
              }`}>{count}</span>
            </button>
          );
        })}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
        {filtered.map(plugin => {
          ...
          return (
            <div
              key={plugin.id}
              className={`glass rounded-xl p-4 transition-all stat-card ${
                plugin.enabled ? 'border-wa-green/10' : 'opacity-60'
              }`}
            >
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-2.5">
                  <div
                    ...
                    style={{
                      backgroundColor: `${catInfo?.color || '#25D366'}15`,
                      color: catInfo?.color || '#25D366'
                    }}
                  >
                  ...
                  <div>
                    ...
                    <code className="text-[10px] text-text-muted font-mono bg-bg-input px-1.5 py-0.5 rounded">
                      {plugin.usage}
                    </code>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="ml-2 px-3 py-1.5 rounded-lg bg-wa-green/10 text-wa-green text-xs font-medium"
                  >
                    Execute
                  </button>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3 text-[10px] text-text-muted">
                  <span className="flex items-center gap-1">
                    {plugin.enabled ? <CheckCircle2 className="w-3 h-3 text-wa-green" /> : <XCircle className="w-3 h-3 text-accent-red" />}
