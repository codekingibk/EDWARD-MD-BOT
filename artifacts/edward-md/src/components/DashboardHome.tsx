...
function StatCard({ icon: Icon, label, value, subtext, color, trend }: { icon: any; label: string; value: string; subtext?: string; color: string; trend?: 'up' | 'down' }) {
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="glass rounded-2xl p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-4 text-sm">
          <div className="text-center"><p className="text-lg font-bold text-text-primary">{formatUptime(stats.uptime)}</p><p className="text-xs text-text-muted">Uptime</p></div>
          <div className="w-px h-8 bg-border" />
          <div className="text-center"><p className="text-lg font-bold text-text-primary">{Math.round(stats.avgResponseTime)}ms</p><p className="text-xs text-text-muted">Avg Response</p></div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard icon={Globe} label="Bandwidth Used" value={`${stats.bandwidthUsed.toFixed(2)} GB`} color="blue" />
      </div>

      {/* Charts Row */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        {[
          { icon: BarChart3, title: 'Messages', data: chartData1, color: '#25D366', iconColor: 'text-wa-green' },
          { icon: Activity, title: 'Commands', data: chartData2, color: '#3b82f6', iconColor: 'text-accent-blue' },
          { icon: TrendingUp, title: 'Users', data: chartData3, color: '#8b5cf6', iconColor: 'text-accent-purple' },
        ].map(({ icon: Icon, title, data, color, iconColor }) => (
          <div key={title} className="glass rounded-2xl p-5">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2"><Icon className={`w-4 h-4 ${iconColor}`} /><h3 className="text-sm font-semibold text-text-primary">{title}</h3></div>
              <span className="text-xs text-text-muted">Last 12h</span>
            </div>
            <MiniChart data={data} color={color} />
            <div className="flex justify-between mt-2 text-xs text-text-muted"><span>12h ago</span><span>Now</span></div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="glass rounded-2xl p-5">
          <h3 className="text-sm font-semibold text-text-primary mb-4 flex items-center gap-2"><HardDrive className="w-4 h-4 text-accent-cyan" />System Health</h3>
          <div className="space-y-4">
            {[
              { icon: Cpu, label: 'CPU Usage', value: stats.cpuUsage, threshold: 80, colorHigh: 'bg-accent-red', colorMid: 'bg-accent-orange', colorLow: 'bg-wa-green', textHigh: 'text-accent-red', textLow: 'text-wa-green', displayMax: 100 },
              { icon: HardDrive, label: 'Memory Usage', value: stats.memoryUsage, threshold: 80, colorHigh: 'bg-accent-red', colorMid: 'bg-accent-orange', colorLow: 'bg-accent-blue', textHigh: 'text-accent-red', textLow: 'text-accent-blue', displayMax: 100 },
              { icon: Clock, label: 'Response Time', value: stats.avgResponseTime, threshold: 300, colorHigh: 'bg-accent-red', colorMid: 'bg-accent-red', colorLow: 'bg-wa-green', textHigh: 'text-accent-red', textLow: 'text-wa-green', displayMax: 500 },
            ].map(({ icon: Icon, label, value, threshold, colorHigh, colorMid, colorLow, textHigh, textLow, displayMax }) => (
              <div key={label}>
                <div className="flex justify-between text-xs mb-1.5">
                  <span className="text-text-secondary flex items-center gap-1.5"><Icon className="w-3 h-3" />{label}</span>
                  <span className={value > threshold ? textHigh : textLow}>{Math.round(value)}{label.includes('Time') ? 'ms' : '%'}</span>
                </div>
                <div className="w-full bg-bg-input rounded-full h-2 overflow-hidden">
                  <div className={`h-full rounded-full transition-all duration-500 ${value > threshold ? colorHigh : value > threshold * 0.6 ? colorMid : colorLow}`} style={{ width: `${Math.min((value / displayMax) * 100, 100)}%` }} />
                </div>
              </div>
            ))}
          </div>
          <div className="grid grid-cols-2 gap-3 mt-5 pt-4 border-t border-border">
            <div className="bg-bg-input rounded-xl p-3 text-center"><p className="text-lg font-bold text-text-primary">{getTotalUsage(plugins).toLocaleString()}</p><p className="text-xs text-text-muted">Total Plugin Uses</p></div>
            <div className="bg-bg-input rounded-xl p-3 text-center"><p className="text-lg font-bold text-text-primary">{successCount}/{errorCount}</p><p className="text-xs text-text-muted">Success/Errors</p></div>
          </div>
        </div>

        <div className="glass rounded-2xl p-5">
          <h3 className="text-sm font-semibold text-text-primary mb-4 flex items-center gap-2"><Activity className="w-4 h-4 text-wa-green" />Recent Activity</h3>
      ...
      {/* Top Plugins */}
      <div className="glass rounded-2xl p-5">
        <h3 className="text-sm font-semibold text-text-primary mb-4 flex items-center gap-2"><TrendingUp className="w-4 h-4 text-accent-pink" />Top Used Plugins</h3>
